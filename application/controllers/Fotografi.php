...
class Fotografi extends CI_Controller {
	...
	public function index()
	{
		...
		foreach ($photos_key as $photo_key) {
			$photos[0]->{$photo_key} = $this->model_photos->findById($pom[0]->{$photo_key});
			var_dump($this->model_photos->findById($pom[0]->{$photo_key}));
		}
		$pom->photos = $photos;
		$data['imgs'] = $photo;
	...
	private function insertProjectContributor_($user_id, $project_id)
	{
		$data = array(
        'user_id' => $user_id,
