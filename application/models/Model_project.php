...
class Model_project extends ORM {
  ...
  protected $contract = ["id", "title", "year", "description", "group_id","photo", "link"];
  ...
  public function findByUserId($id) {
    $query = "SELECT `projects`.*, `projects_groups`.`title` as `group_title` FROM `projects` JOIN `projects_groups` ON `projects`.`group_id`=`projects_groups`.`id` WHERE `projects`.`id` IN (SELECT `project_id` FROM `projects_contributors` WHERE user_id = ?)";
